...
// This script lives in <project>/scripts, not inside geoip-lite's own
// scripts/ folder like the original it's based on — every path that assumed
// __dirname was node_modules/geoip-lite/scripts has to point there
// explicitly instead.
var geoipLitePkgDir = path.resolve(__dirname, '..', 'node_modules', 'geoip-lite');
var utils = require(path.join(geoipLitePkgDir, 'lib', 'utils'));
var Address6 = require('ip-address').Address6;
var Address4 = require('ip-address').Address4;

// MaxMind retired the query-parameter download endpoint (?license_key=...)
// this script originally used; it now requires HTTP Basic Auth with an
// account ID plus the license key against a different URL. See
// getHTTPOptions() below for where the Authorization header actually gets
// added — only for requests to download.maxmind.com itself, not for the
// signed redirect to MaxMind's storage backend that follows.
var accountId = process.env.MAXMIND_ACCOUNT_ID;
var licenseKeyValue = process.env.MAXMIND_LICENSE_KEY;
if (!accountId || !licenseKeyValue) {
	// Runs from postinstall on every `npm install`, including for anyone
	// (local dev, CI) who never set these — a hard failure here would break
	// every install everywhere. Skipping quietly just leaves whatever
	// GeoIP data npm already installed (the stale bundled snapshot) in
	// place, same as if this script didn't exist.
	console.log('Skipping GeoIP data update: MAXMIND_ACCOUNT_ID / MAXMIND_LICENSE_KEY not set.');
	process.exit(0);
}
...
var dataPath = path.join(geoipLitePkgDir, 'data');
...
var tmpPath = process.env.GEOTMPDIR ? process.env.GEOTMPDIR : path.resolve(__dirname, '..', '.geoip-update-tmp');
var countryLookup = {};
var cityLookup = {NaN: -1};
// No more per-database checksum URL — the current API doesn't expose the
// old .sha256 suffix, so "check" below just always treats the database as
// needing a (re)download instead of skipping unchanged ones. This runs as
// an occasional manual/scheduled task, not on every request, so the extra
// download cost of not skipping is not a real concern.
var databases = [
	{
		type: 'country',
...
function getHTTPOptions(downloadUrl) {
	...
	// Only the initial request to MaxMind's own API needs our credentials —
	// it 302s to a pre-signed storage URL on a different host, and that URL
	// carries its own auth in the query string. Sending Basic Auth there too
	// is unnecessary and (depending on the storage backend) can outright
	// break a request that's already fully authorized by its signature.
	if (parsedUrl.hostname === 'download.maxmind.com') {
		options.auth = accountId + ':' + licenseKeyValue;
	}
...
// (credential check already happened above, right after reading env vars)
